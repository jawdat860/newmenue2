...
const ModelLink = ({ services, activeCategory, activeSubcategory }) => {
  ...
  return (
    <Modal
      ...
      dismissible={true}
      trigger={
        <button className="mx-[6px] px-4 py-2 text-sm font-medium bg-primary text-white rounded-full transition-all duration-300 active:bg-primary active:opacity-50 focus:outline-none">
          <FiAlignJustify />
        </button>
      }
    ...
    >
      ...
      <div
        ...
        className="p-4 mt-[20px] flex-[1] text-black overflow-scroll "
      >
        <ul
              ...
              <li key={service.id} className="p-2">
                {service.subcategories && service.subcategories.length > 0 ? (
                  ...
                  <ScrollLink
                    ...
                    duration={500} // Duration of scroll animation
                    offset={-100} // Optional: Offset the scroll position (e.g., for sticky headers)
                    className={`text-black block hover:underline cursor-pointer ${
                      activeCategory === service.id
                        ? "font-bold p-[8px] text bg-primary rounded-[10px]"
                        : ""
                    }`} // Highlight active link
